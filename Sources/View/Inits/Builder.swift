...
//        )
		...
		public static func buildIf<T: View>(_ content: T?) -> ConstructionItem {
				return ConstructionItem(contentViews: content?.extractContentViews() ?? [])
		}

		public static func buildOptional<T: View>(_ view: T?) -> ConstructionItem {
				if let view = view {
						return ConstructionItem(contentViews: view.extractContentViews())
				}
				return ConstructionItem(contentViews: [])
		}
}
